...
<div class="container wide-container mt-5">
  ...
  <div class="row mb-3">
    <div class="col-md-4">
      <form method="GET" action="{{ route('management.patient.search') }}" class="input-group">
  ...
  <table class="table table-hover">
    ...
    <tbody>
      @foreach($patients as $patient)
        <tr style="cursor:pointer;" onclick="window.location='{{ route('management.history', $patient->id) }}'">
          <td>{{ $patient->name }}</td>
          <td>{{ $patient->id }}</td>
        </tr>
      @endforeach
